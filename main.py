...
## load data
df_train = pd.read_csv('data_train.txt',sep='\t')
df_test = pd.read_csv('data_test.txt',sep='\t')
...
all_data.drop(['D5','D9','D11','D17','D22','D28'],axis=1,inplace=True)
...
data_minmax['D0'] = data_minmax['D0'].apply(lambda x:math.exp(x))
data_minmax['D1'] = data_minmax['D1'].apply(lambda x:math.exp(x))
data_minmax['D6'] = data_minmax['D6'].apply(lambda x:math.exp(x))
data_minmax['D30'] = np.log1p(data_minmax['D30'])
...
np.savetxt('final_save.txt',final)
print('File saved!')
